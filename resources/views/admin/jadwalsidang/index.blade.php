...
<main>
    ...
    <a href="/jadwal-ta-admin/create" class="btn btn-primary mb-3">Add Jadwal</a> 
    @if(session('error'))
    <script>
        alert('{{ session('error') }}');
    </script>
@endif
    <table class="table table-boreder table-striped">
        ...
        <tr>
            ...
            <td>
                <a href="/jadwal-ta-admin/{{$ta->id}}/edit" class="btn btn-success btn-sm">Edit</a>
                {{-- <a href="" class="btn btn-danger btn-sm">Hapus</a> --}}
                <form action="jadwal-ta-admin/{{$ta->id}}" method="post" class="d-inline">
                    @method('DELETE')
                    @csrf
